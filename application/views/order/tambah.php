                  ...
                  <div class="col-md-6 form-group">
                    ...
                    <select name="id_pelanggan" class="form-control" id="select-pelanggan" data-live-search="true">
                      ...
                      <?php foreach ($pelanggan as $key) { ?>
                        <option value="<?= $key['id_pelanggan'] ?>" <?= set_value('id_pelanggan') == $key['id_pelanggan'] ? 'selected' : '' ?>><?= $key['nama_pelanggan'] ?></option>
                      <?php } ?>
                    </select>
                  ...
                  <div class="col-md-6 form-group">
                    ...
                    <select name="id_produk" class="form-control" id="select-produk" data-live-search="true">
                      ...
                      <?php foreach ($produk as $key) { ?>
                        <option value="<?= $key['id_produk'] ?>" <?= set_value('id_produk') == $key['id_produk'] ? 'selected' : '' ?>><?= $key['nama_produk'] ?> - Rp <?= number_format($key['harga_produk'], 0, '.', ',') ?></option>
                      <?php } ?>
                    </select>
                    <?= form_error('id_produk', '<span class="text-danger small">', '</span>'); ?>
                  </div>
